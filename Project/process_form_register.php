...
if(!empty($_POST)){
	...
	if(empty($fullname) || empty($email) || empty($pwd) || strlen($pwd) < 6){
		...
		if($userExist != null){
            ...
            // su dung ma hoa 1 chieu -> ma hóa MD5 -> hack
            $pwd = getSercurityMD5($pwd);

			$sql = "insert into user(fullname, email, password, role_id, created_at, updated_at, 
			        deleted) values ('$fullname', '$email', '$pwd', 2, '$created_at', '$updated_at', 
			        0)";
